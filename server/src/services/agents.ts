  ...
  return {
    ...
    remove: async (id: string) => {
      ...
      return db.transaction(async (tx) => {
        ...
        await tx.execute(sql`UPDATE issues SET checkout_run_id = NULL, execution_run_id = NULL WHERE checkout_run_id IN (SELECT id FROM heartbeat_runs WHERE agent_id = ${id})`);
        ...
        await tx.execute(sql`DELETE FROM activity_log WHERE run_id IN (SELECT id FROM heartbeat_runs WHERE agent_id = ${id})`);
        await tx.execute(sql`DELETE FROM activity_log WHERE agent_id = ${id}`);
