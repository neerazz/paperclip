...
function buildWakeText(payload: WakePayload, paperclipEnv: Record<string, string>): string {
  ...
  const lines = [
    "Paperclip heartbeat wake. Execute this procedure now.",
    "",
    "Environment:",
    ...envLines,
    `PAPERCLIP_API_KEY=<load from ${claimedApiKeyPath}>`,
    "",
    "Auth: Bearer $PAPERCLIP_API_KEY on every request. X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID on mutations.",
    "",
    "Procedure:",
    "1. GET /api/agents/me",
    `2. If PAPERCLIP_TASK_ID set: POST /api/issues/{id}/checkout body: {"agentId":"$PAPERCLIP_AGENT_ID","expectedStatuses":["todo","backlog","blocked"]}`,
    "   GET /api/issues/{id} and GET /api/issues/{id}/comments, execute the work.",
    "   POST /api/issues/{id}/comments if needed. PATCH /api/issues/{id} with status=done when complete.",
    "3. If no task: GET /api/companies/$PAPERCLIP_COMPANY_ID/issues?assigneeAgentId=$PAPERCLIP_AGENT_ID&status=todo,in_progress,blocked then pick highest priority and execute step 2.",
    "4. To create issues: POST /api/companies/{companyId}/issues",
    "",
